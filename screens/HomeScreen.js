...
export default class HomeScreen extends React.Component {
  ...
  constructor(props){
    super(props);
    this.state = {
      data : [],isLoading : true, isListEmpty : false
    }
  }
  // lifecycle method
  componentWillMount() {
  ...
  getAllContact = () => {
    ...
    contactRef.on("value",dataSnapshot => {
      if(dataSnapshot.val()){
        ...
        let contactKey = Object.keys(dataSnapshot.val()) //to get keys
        contactKey.forEach((value,key) => {
          contactResult[key]['key'] = value //saving keys with data
        })
        //TODO:
        // sort array by fname and set it to data state
        self.setState({ data : contactResult.sort( (a,b) =>{
